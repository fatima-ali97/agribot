...
const chart = Highcharts.chart("chart2", {
  chart: {
    ...
    events: {
      load: function () {
        ...
        const series = this.series[0]; // Only one series for air quality
        setInterval(() => {
          fetch("/airQuality")
            ...
            .then((csv) => {
              ...
              const airQualityPoint = latest[0]; // Second column is Humidity

              // Add the point to the series
              series.addPoint([x, airQualityPoint], true, true);
            })
  ...
  title: {
    text: "air Quality levels (PPM)",
  },
  subtitle: {
    text: "numbers are taken from the mq135 sensor",
  },
  xAxis: {
    type: "datetime",
